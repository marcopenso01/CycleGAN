"""
Created on Sat Jan 16 15:12:49 2021
...
if __name__ == '__main__':
    ...
    tit=['artefact contour', 'ventricle DX contour', 'ventricle SX contour']
    for file, cc in zip(sorted(os.listdir(paz_path)), range(len(os.listdir(paz_path)))):
        ...
        for ii in range(3):
            img = cv2.imread(addr)
            #img = Image.open(addr)
            clahe = cv2.createCLAHE(clipLimit = 1.4)
            ...
            while(1):
                ...
                if k==27: #Escape KEY
                    if ii==0:
                        ...
                        im_out2 = imfill(image_binary)
                        im_out2[im_out2>0]=255
                        cv2.imwrite(os.path.join(paz_out, 'ventricle DX', file), im_out2)
